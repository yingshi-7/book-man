...
**在view/man文件夹中新建页面BorrowMan.vue**
...
### 18.3 调用借阅图书信息展示的数据，并双向绑定

**在api/borrow.js中提供查阅所有借阅图书信息的接口**
...
**src/stores/userInfo.js中，定义个人中心状态**
...
个人信息之前已经存储在pinia中，所以在页面加载完后，直接从pinia中获取个人信息，并回显到表单中
...
提供updateAvatar函数,调用接口上传头像

```js
// 调用接口，更新头像
import {uploadAvatarUpdateService} from '@/api/user.js'
import { ElMessage } from 'element-plus';
